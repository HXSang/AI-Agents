"""Monthly Insight Service Implementation.

Single-file implementation of the monthly insight pipeline, mirroring the
flat-file convention used by other ``*_service_impl.py`` modules in this
package (chat, insight, financial, health…).

Pipeline (in ``MonthlyInsightServiceImpl.generate_monthly``):

    0.  Resolve target month + time-gating: day 1-2 (user tz) redirects to last
        month and sets the final-day window; otherwise today's day-of-month
        picks the active phase (3-10 → P1 / 11-20 → P2 / 21-EOM → P3; a completed
        past month → P3). A completed past month also short-circuits here by
        returning its cached response verbatim (skips steps 1-12).
    1.  Fetch daily snapshots from BE pipeline           (external_api)
    2.  Split month into 3 phases (1-10 / 11-20 / 21-EOM)
    3.  Monthly rollup + 7 cross-module Pearson corr + baseline context
    4.  Detect patterns across 7 families (A/B/C/D/E/F/X) + historical, then
        TIME-GATE: day 1-2 → only the 5 final-day patterns; every other day →
        drop final-day patterns and keep the active phase's patterns plus the
        whole-month / cross / Family A-F patterns.
    5.  Quality gates, edge-case filter, intelligence layers
        (context / confidence / memory / tone)
    6.  Anti-repetition lookup (Redis) → novelty score per candidate
    7.  Score = 0.4·signal + 0.3·novelty + 0.3·cross_module    (Rules §5)
    8.  Dedupe by insight_hash (priority + score)
    9.  Top-N pick: 1 per category + week-based family rotation (Rules §4)
    10. Render text per insight (LLM-first, template fallback) + similarity filter
    11. Best-effort persist to Redis (snapshot + insights anti-rep + past-month
        view cache)
    12. Project to {great_job, need_attention, opportunity} + translate

The file is intentionally flat to match the codebase pattern of 1 ABC
interface + 1 monolithic ``_impl.py`` per service.
"""
...
from services.monthly_insight_service import MonthlyInsightService
...
class BaselineContext:
    """Past-month statistics used by historical detectors.

    Populated by ``MonthlyInsightServiceImpl._load_baseline_context`` from
    Redis snapshots saved by ``_save_monthly_snapshot``. All fields are
    optional — detectors must gracefully fall through when data is missing.

    ``prev_month_snapshot`` — last month's full MonthlySnapshot (drives
        carry-over, level-vs-history, improvement/deterioration rules).
    ``prev_month_insights`` — last month's rendered insights (drives
        positive/negative carry-over per Rules §1 P1 rows 9-10).
    ``avg_90d`` / ``avg_180d`` — averaged key metrics across past N months
        (drives habit alignment and behaviour shift rules).
    ``past_insights_by_month`` — past-month → Insight list map; drives 90d
        pattern repeat (P2 row 12) and reinforcement (P3 row 12) detectors.
    """
...
# Key MonthlySnapshot fields averaged into ``avg_90d`` / ``avg_180d``. Kept
# small so JSON payload stays compact; extend cautiously.
...
# Work-day span in minutes used as proxy for "events after 9pm" (Rules §3.B row 1).
...
_VARIANCE_KEYS: Tuple[str, ...] = (
    ...
    # Phase-specific std-dev raw_signal keys emitted by the variance-flavoured
    # Need-Attention detectors (irregular_finish / increasing_variability /
    # stress_pattern). Without these, ``_downgrade_low_volume`` misses the
    # very candidates Rules §5 row 5 targets.
...
# Rules §5 row 3 — "Low activity across all → Opportunity, not Need Attention".
# Trigger when the monthly overall_day_score average sits at/under this floor
# (BE default for a zeroed module is ≈50, so anything ≤55 indicates the user
# is not really engaging with most modules across the month).
...
# Anti-rep lookback in months
ANTI_REP_LOOKBACK_MONTHS: int = 3

# Rules §1 Phase 2 row 4 "Stable rhythm" — low variance ACROSS MODULES in P2.
# Spec example: "Your routine is remaining relatively steady across different areas."
# Threshold: std-dev across 4 module scores in P2 (on 0-100 scale). Derivation:
# 10 points on a 100-point scale = 10% spread, INSIDE the Rules §0 row 7 floor
# "Only flag strong signals if deviation > 20-30%" — i.e., variance must stay
# BELOW that floor to qualify as "stable".
_STABLE_RHYTHM_P2_MAX_STD: float = 10.0

# Rules §1 Phase 2 row 6 "Sustained balance" — spec literally says
# "Balance score across Phase 1 & 2 remains above 75% on average".
# balance_score is now mean(h_health_score, productivity_score,
# financial_health_score) on 0-100 scale per spec. Threshold: average of
# Phase 1 & Phase 2 balance must clear 75 (out of 100).
_SUSTAINED_BALANCE_MIN_AVG: float = 75.0
# Rules §1 Phase 2 row 7 "Growing imbalance" — when interpreted with the
# mean-based balance_score, "imbalance growing" maps to "level dropping from
# Phase 1 to Phase 2". Threshold: Phase 1 balance MINUS Phase 2 balance ≥ 20
# (on 0-100 scale = 20 percentage points of overall level decline). Aligned
# with Rules §0 row 7 "deviation > 20-30%".
_PHASE_BALANCE_DROP_MIN: float = 20.0
# Calendar suppression for §5 row 1 — "Days 1-5 only → suppress Need Attention".
# Keyed on calendar day-of-month, not data sparsity.
_EARLY_CALENDAR_DAY_LIMIT: int = 5
# Rules §3.A row 1 "Meeting overload" — need ≥3 days where meetings > 6h.
_MEETING_HEAVY_DAY_THRESHOLD_MIN: float = 360.0
_MEETING_HEAVY_MIN_DAYS: int = 3
# Rules §1 Phase 2 row 9 "Rebalancing — health ↑ after earlier drop". Phase 1
# health must drop ≥ 20% vs prev-month / 90d baseline before P2 rebound counts
# as a real recovery (otherwise any building momentum would mislabel as
# rebound). Aligned with Rules §0 row 7 "deviation > 20-30%" floor.
_RECOVERY_REBOUND_BASELINE_DROP_PCT: float = -0.20

# Rules §1 FINAL DAY rows 1-4 — month-end synthesis detectors. Only fire when
# the month is essentially complete (≥25 days with data) so the wording
# ("In April 2026, your pattern…") is honest.
...
# Final-Day row 5 historical comparison — drop this far below baseline escalates
# the card from Opportunity to Need Attention (so the pattern covers "All" buckets).
_FINAL_DAY_HISTCOMP_NA_DROP: float = 0.35

# Rules §4 vs §1 Phase 1 rows 9-10, Phase 2 row 12, Phase 3 rows 12-13:
# Carry-over / pattern-repeat / habit detectors EXPECT repetition across months
# — repetition IS the signal. Exempt them from the standard novelty penalty so
# anti-rep does NOT conflict with "same pattern as last month = Great Job".
_CARRY_OVER_EXEMPT_TYPES: frozenset = frozenset(
    {
        ...
        # Final-Day synthesis (Rules §1 Final-Day rows 1-5) — these are by
        # design retrospective month-end observations that recur every month
        # with the same underlying signal. Penalising them as repetition
        # contradicts the spec intent of surfacing persistent patterns.
...
# Rules §4 Rule 1 — week-based family rotation. Map week-of-month (1..4+) to
# the family that should be FAVOURED in the top-N pick this week. Past months
# (or month-end synthesis) get no boost.
...
# Additive boost applied to in-focus family score during top-N pick.
...
# Rules §3.A row 3 — "Many short events (<30 mins)" defines context_switching.
...
# Family B — Time-of-day
# Rules §3.B row 1 "Late work impact" — spec phrases "Events after 9pm" without
# a numeric frequency. We require ≥30% of days to exhibit a long work-day span
# so a one-off late evening doesn't trip the detector. 30% = at least 2-3 days
# in a typical 7-10 day Phase, balancing recall vs §0 row 7 "strong signals only".
...
# Rules §3.C row 3 "Busy day → higher spending" (Opportunity) — spec gives no
# numeric. We split the work-spend correlation domain into three non-
# overlapping bands so the three finance-context detectors don't collide:
#   busy_day_spending      ∈ [0.30, 0.40)  — Opportunity (mild link)
#   event_driven_spending  ∈ [0.40, 0.50)  — Opportunity (moderate)
#   stress_linked_spending ≥ 0.50          — Need Attention (strong)
...
# Rules §1 Phase 2 row 2 — "Phase 2 > Phase 1 by >15%" → Opportunity (building momentum).
...
# Rules §1 Phase 1 row 5 — "Balanced start: All modules within ±15% range" → Great Job.
# Uses ``module_max_excess_pct`` (max-vs-others excess fraction); ≤0.15 means
# no module is more than 15% above the mean of others.
_BALANCED_START_MAX_EXCESS: float = 0.15

# Phase 2 / Phase 3 balance & variance (Rules §1)
# Rules §1 — high-imbalance threshold. With balance_score now mean-based, the
# imbalance signal is the STDDEV of the same 3 module scores (h_health,
# productivity, financial_health) on 0-100 scale. A std of 15 = "modules
# differ by ~15 points on average" → flagged as imbalance. Used in:
#   - ``_cross_module_imbalance`` (P3 row 9): fires when imbalance_std ≥ this → NA
#   - ``_carry_over_pattern_holds`` / ``_negative_carry_over``: carry-over
#     guards consume this same threshold.
...
# Phase variance gaps (Rules §1 Phase 1 row 6 / Phase 2 row 5 / Phase 2 row 11)
...
# =============================================================================
# Historical baseline thresholds (Rules §1 Phase 1 rows 1-3, 9-11; Phase 2
# row 3, 12-13; Phase 3 rows 10-13; Final-day row 5)
# =============================================================================

# Phase 1 — Level vs history (rows 1-3)
...
# Phase 1 — Carry-over (rows 9-10) — same category in last month → confidence boost
_CARRY_OVER_MIN_SIGNAL: float = 0.40  # last-month insight must have score ≥ this
# Rules §1 P1 row 9 "same STRONG pattern" — carry-over fires in Phase 1
# (Day 3-10) so only patterns describing a durable behaviour that can
# meaningfully persist across the month boundary qualify. Phase-3 / Final-Day
# specific patterns are intentionally excluded — they describe end-of-month
# behaviour and cannot semantically carry into a new month's Phase 1.
...
# Rules §1 P1 row 10 "same IMBALANCE" — only NA patterns whose spec wording
# explicitly describes cross-module imbalance qualify. Final-Day synthesis
# patterns are excluded because they are retrospective and cannot carry
# forward into Phase 1 of the next month.
...
# Phase 1 — 180d habit alignment (row 11)
_HABIT_180D_MATCH_PCT: float = 0.10  # current within ±10% of 180d avg = aligned

# Phase 2 — Divergence vs history (row 3)
_DIVERGENCE_VS_HISTORY_PCT: float = 0.20  # ≥20% different from prev month mid

# Phase 2 — 90d pattern repeat (row 12)
_PATTERN_REPEAT_MIN_OCCURRENCES: int = 2  # ≥2 of last 3 months show same pattern

# Phase 2 — 180d behaviour shift (row 13)
_BEHAVIOUR_SHIFT_PCT: float = 0.20  # ≥20% deviation from 180d baseline

# Phase 3 — Improvement / deterioration vs last month (rows 10-11). balance_score
# is mean(3 modules) on 0..100 scale; threshold = 10 points of change between
# months (≈ 10% of full-scale).
_MONTH_CHANGE_BAL_PCT: float = 10.0  # balance change vs prev month threshold

# Phase 3 — 90d reinforcement (row 12). Spec: "Same outcome across 3 months".
# We count occurrences in the past 3 months and REQUIRE the current month to
# also show the pattern (verified via ``_pattern_holds_in_current_month``).
# So the past-month floor is 2 — combined with the current month, total = 3.
# Single source of truth for the past-month threshold (avoids the prior
# off-by-one indirection of subtracting from a "total months" constant).
_REINFORCEMENT_PAST_MIN: int = 2

# Phase 3 — 180d habit (row 13) — same alignment check as Phase 1 habit
# Reuses ``_HABIT_180D_MATCH_PCT``

# Final-day historical comparison (row 5) — same thresholds as Phase 2 divergence

# Lookback window — months of past MonthlySnapshot kept for baseline computation.
# Set to 6 to cover both 90d (3) and 180d (6) baselines per spec.
...
def safe_pearson(
    ...
    """Pearson r rounded to 3 decimals.

    Returns ``None`` when:
      * fewer than 3 valid (non-None on both sides) pairs,
...
def pct_delta(new: Optional[Number], old: Optional[Number]) -> Optional[float]:
    """``(new - old) / |old|`` rounded to 4 decimals.

    Returns ``None`` when either side is missing or ``old == 0``.
    """
...
def _module_balance(
    ...
    """Module balance — per spec: mean(h_health, productivity, financial_health).

    Per Rules §1 (Cross-module logic) + product spec: balance_score is the
    arithmetic mean of THREE composite module scores. Mood is intentionally
    excluded — spec only mentions mood in Family C sequence patterns
    (sleep→work→mood, workout→mood→productivity), not in any balance pattern.

    Per Rules §5 row 2: missing module data → exclude from balance calculation.
    Requires ≥2 of the 3 modules to have data; otherwise returns all-None.

    Returns:
        balance_score: arithmetic mean of available modules in 0..100 scale.
            HIGH = doing well overall, LOW = under-performing overall.
        concentrated: name of module with highest score (for skewed_allocation
            template).
        module_scores: dict {module_name: raw score 0..100} — kept un-normalized
            so downstream detectors can compare directly with module fields.
        max_excess_pct: (max_module - mean(others)) / mean(others), used for
            Rules §1 Phase 1 row 4
                        "Skewed allocation: One module >30% higher than others"
    """
...
# Rules §4 Rule 3: "Score insights by Uniqueness × Strength × Recency".
# Spec doesn't quantify the recency curve, so we use the geometric decay 1.00 /
# 0.50 / 0.25 (halving per month back) and a 0.5 scale so the curve degenerates
# to the legacy binary novelty (1.0 / 0.5 / 0.0) when only 1 recent match
# exists. This preserves backwards compatibility while spreading the penalty
# over a 3-month window per §4 anti-rep lookback (``ANTI_REP_LOOKBACK_MONTHS``).
_RECENCY_PENALTY_BY_AGE: Dict[int, float] = {
    1: 1.00,  # last month — strongest penalty
    2: 0.50,
    3: 0.25,
}
...
def novelty_score(
    ...
    """Rules §4 Rule 3 — Recency-weighted novelty.

    Returns 1.0 (no prior matches) down to 0.0 (heavy recent overlap).

    Carry-over / habit / pattern-repeat detectors (``_CARRY_OVER_EXEMPT_TYPES``)
    always score 1.0 — repetition IS their signal per Rules §1 Phase 1 rows
    9-11, Phase 2 rows 12-13, Phase 3 rows 12-13.

    Recency weighting (when ``past_insights_by_month`` + ``current_year`` /
    ``current_month`` provided):
        novelty = max(0.0, 1.0 - 0.5 × Σ penalty(match, age))

    where ``penalty`` is 1.00 for last-month matches, 0.50 for 2-month-old,
    0.25 for 3-month-old, 0 beyond.

    Backwards-compatible: when called with only ``recent_insights`` (flat list,
    no month context), falls back to legacy binary scoring so existing tests
    and call paths keep working.
    """
...
def compute_score(candidate: InsightCandidate, novelty: float) -> float:
    """Rules §5 Insight Scoring — ``0.4·signal + 0.3·novelty + 0.3·cross_module``.

    The "Primary insight" ordering in spec §2 (Cross-phase synthesis Step 1)
    asks for "strongest signal" but the engine combines strength with novelty
    and cross-module impact in a single score so the top-N pick (a) does not
    repeat last month's top insight verbatim and (b) prefers cross-module
    stories per Rules §7 "context-aware stories". This is the deliberate
    interpretation of §2 within the published scoring formula.
    """
    return round(
...
def _infer_trigger(candidate: InsightCandidate) -> str:
    """Infer trigger type from candidate fields (Rules §5 row 4 conflict priority).

    Spec demands "correlation > trend > level" but only a handful of detectors
    set ``raw_signal['trigger']`` explicitly. Inferring from field names lets
    every detector participate without per-detector annotation:

    * Any ``corr_*`` key ⇒ ``correlation`` (cross-module Pearson)
    * Any ``*_delta`` / ``*_vs_*`` key ⇒ ``trend`` (phase-to-phase or month-to-month)
    * Otherwise ⇒ ``level`` (absolute threshold)

    Note: ``*_pct`` alone is NOT enough to imply trend — many level-based
    detectors report proportions (e.g. ``module_max_excess_pct``,
    ``overdue_pct``). Trend requires an explicit comparison key.

    Explicit ``raw_signal['trigger']`` (legacy 4 detectors) still wins so we
    can tag ``phase_rise`` / ``phase_deterioration`` specifically.
    """
...
# Rules §5 row 4 — additive score modifier so "correlation > trend > level"
# influences both ``_dedupe`` tie-break (legacy) AND the top-N pick. Magnitude
# smaller than ``_WEEK_FOCUS_BOOST`` so week rotation stays dominant.
...
def _apply_conflict_priority_boost(
    ...
    """Apply Rules §5 row 4 conflict-priority boost to scores.

    Without this step, ``_pick_one_per_category`` purely picks by score and
    spec's correlation-over-trend-over-level ordering only activates on the
    near-impossible ``_dedupe`` tie-break. With the boost, correlation
    detectors gain a small structural edge when competing for the same slot.
    """
    if not scored:
        return scored
...
# Rules §1 Final-Day — the 5 month-end synthesis patterns. In the day-1-2
# window these are the ONLY patterns surfaced (last month's wrap-up); every
# other pattern is suppressed.
...
# Days-of-month (user tz) that count as the "1st-2nd of the following month"
# window where only last month's final-day synthesis is shown.
_FINAL_DAY_WINDOW_MAX_DOM: int = 2

# Calendar-day boundaries for TIME-GATED detection (mirrors ``_PHASE_BOUNDS``
# but keyed on TODAY in the user tz). Days 1-2 = final-day window (handled
# above); 3-10 → Phase 1, 11-20 → Phase 2, 21-EOM → Phase 3. On any given day
# only the patterns of the active phase (plus whole-month / cross / Family A-F
